...
moisture <- fW.RothC(refdata$Precip, refdata$`Evp$Evp`, S.Thick = 25, pClay = 48, pE = 1, bare = FALSE) # pE is pan evaporation
# bare = true or false?
...
tempeff <- fT.RothC(refdata$Temp)
```

External effect from moisture and temperature effect

```{r}
xi = data.frame("month"=1:1212 , "xi"=moisture$b*tempeff)

# model for reference data
refmodel <- RothCModel(xi$month, ks = c(k.DPM = 10, k.RPM = 0.3, k.BIO = 0.66, k.HUM = 0.02, k.IOM = 0), C0 = c(0, 0, 0, 0, FallIOM), In = 2.7, DR = 1.44, clay = 48, xi = xi, solver = deSolve.lsoda.wrapper, pass = FALSE)

cstockref <- getC(refmodel)
